...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
    const string& wordEmbFile, const string& charEmbFile, const string& c2_devoutFile, const string& c2_testoutFile) {
  if (optionFile != "")
  ...
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    if (devNum > 0) {
      ...
      stringstream ss;
      ss <<iter;
      m_pipe.outputAllInstances(c2_devoutFile + "." + ss.str(), decodeInstResults);
      ...
      if (testNum > 0) {
        ...
        metric_test.print();
         ...
         if(true){
          m_pipe.outputAllInstances(c2_testoutFile + "." + ss.str(), decodeInstResults);
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  std::string trainFile = "", devFile = "", testFile = "", modelFile = "", c2_devoutFile = "", c2_testoutFile = "";
  ...
  ah.new_named_string("c2_devout", "devoutCorpus_type2", "named_string", "development corpus of type2 to train a model, optional when training", c2_devoutFile);
  ah.new_named_string("c2_testout", "testoutCorpus_type2", "named_string",
      "testing corpus of type2 to train a model or input file to test a model, optional when training and must when testing", c2_testoutFile);
  ...
  Labeler tagger;
  if (bTrain) {
    tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, charEmbFile,c2_devoutFile,c2_testoutFile);
  } else {
