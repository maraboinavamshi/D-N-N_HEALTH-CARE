...
class UnLSTMCRFMLClassifier {
public:
  UnLSTMCRFMLClassifier() {
    _dropOut = 0.5;
  }
  ~UnLSTMCRFMLClassifier() {
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, const NRMat<dtype>& charEmb, int charcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int charhiddensize,
    ...
    _tanh_project.initial(_hiddensize, _lstmhiddensize, true, 55, 0);
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      _tanh_project.ComputeForwardScore(project_left, project);
      ...
      _tanh_project.ComputeBackwardLoss(project_left, project, projectLoss, project_leftLoss);
